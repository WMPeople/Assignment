...
public class VersionManagementService {
	...
	public List<BoardHistory> getRelatedHistory(NodePtr leafPtr) throws NotLeafNodeException {
		...
		Map<Map.Entry<Integer, Integer>, BoardHistory> boardHisotryMap = boardService
	...
	private BoardHistory createArticleAndHistory(Board article, int version, final String status,
		...
		if (version == 0) { // 루트 노드일 경우
			...
			if (insertedRowCnt != 1) {
				...
				throw new RuntimeException("createArticleAndHistory메소드에서 게시글 이력 추가 에러 insertedRowCnt : "
					+ insertedRowCnt + "\nrootHistory : " + json);
			}
